...
function showProduct(filterProducts) {
  cardGroup.innerHTML = "";
  for (const product of filterProducts) {
    const tagsText = product.tags.map(tag => `<span>${tag}</span>`).join('');
    cardGroup.innerHTML += 
    `
      <div class="card">
...
function search() {
  ...
  const filterProducts = products.filter(product => product.title.toLowerCase().includes(searchTerm));
